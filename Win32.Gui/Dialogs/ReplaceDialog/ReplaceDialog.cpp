...
UINT UM_REPLACE_BUTTON_CLICKED = RegisterWindowMessage(FINDMSGSTRING);
...
UINT_PTR OnReplace(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == UM_REPLACE_BUTTON_CLICKED && hwnd == window) return OnReplace(hwnd, message, wParam, lParam);
  return CallWindowProc(defWndProc, hwnd, message, wParam, lParam);
}

int main() {
  window = CreateWindowEx(0, WC_DIALOG, L"ReplaceDialog exemple", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 300, 300, nullptr, nullptr, nullptr, nullptr);
  button1 = CreateWindowEx(0, WC_BUTTON, L"Replace...", WS_CHILD | WS_VISIBLE, 10, 10, 80, 25, window, nullptr, nullptr, nullptr);
